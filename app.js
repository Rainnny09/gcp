...
async function main() {
    const UUID = await getVariableValue('UUID', '6b17a8d8-bb13-4c53-9102-f97de4e22e1a'); // 为保证安全隐蔽，建议留空，可在Node.js界面下的环境变量添加处（Environment variables）,点击ADD VARIABLE，修改变量
    ...
    const DOMAIN = await getVariableValue('DOMAIN', 'my-friend-kenya-71934756675.europe-west4.run.app');// 为保证安全隐蔽，建议留空，可在Node.js界面下的环境变量添加处（Environment variables）,点击ADD VARIABLE，修改变量
